...
#PP not present in vector, returns FALSE
"PP" %in% c("DM", "DO", "DS")
```
...
```{r}
# ! makes the expression return the opposite
!(10 >= 5)
...
# If condition is not met code will not produce a return
```
...
# if writing just an "else" statement, do not need to set new conditions
```
...
# can't tell case_when what to do if something doesn't match like in if else statements
```
