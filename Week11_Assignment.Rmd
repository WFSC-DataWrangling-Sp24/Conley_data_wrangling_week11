...
### 5. Using `dplyr` Choice Functions (20 points)For this question, we will be using functions from `dplyr` and data from the `lterdatasampler` package, so make sure you load both of those packages with the `library()` function.
...
e.  Now use the `case_when` function inside a `mutate` function to create a new column called `size_category3` in the `pie_crab` dataframe that meets the same conditions as (b) and (c).
    ...
    if_else(crab_size >= 20, "large", 
            if_else(crab_size >= 10 & crab_size < 20, "medium", "small"))
